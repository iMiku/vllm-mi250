...
- Merged embeddings enter the text model via ``inputs_embeds``, i.e. before
  ...
  the model so the hash MoE router (``tid2eid`` lookup) sees real token ids
  (``requires_raw_input_tokens``). The reference's per-image-token routing
  bias ``bias_vl`` has no hook in vllm's FusedMoE router and is dropped at
  load time: text-token routing stays exact, image-token expert routing is
  approximate.
"""
...
def _make_deepseek_v4_vl_weights_mapper(
    ...
    return WeightsMapper(
        ...
        orig_to_new_regex={
            # The hash layers (first num_hash_layers == 3 of this checkpoint)
            # also store a ``ffn.gate.bias``; the reference router never
            # applies it to hash routing (``tid2eid`` only), and the vllm hash
            # layers have no bias param for it. Drop it before the suffix rule
            # ``.ffn.gate.bias -> .e_score_correction_bias`` would fire.
            **base.orig_to_new_regex,
            re.compile(r"layers\.[012]\.ffn\.gate\.bias$"): None,
        },
        orig_to_new_suffix={
            "head.weight": "language_model.lm_head.weight",
            "embed.weight": "embed_tokens.weight",
            ".ffn.gate.bias": ".ffn.gate.e_score_correction_bias",
            # The vision variant adds a per-layer ``bias_vl`` routing bias the
            # reference applies to image tokens only. vllm's FusedMoE router
            # has no per-token-type bias hook, so drop it: text-token routing
            # stays exact, image-token expert routing is approximate.
...
class DeepseekV4ForConditionalGeneration(
    ...
    # The hash MoE router needs raw token ids (``tid2eid`` lookup) for every
    # token, including the borrowed image sentinel ids (see
    # common/mm_preprocess.py). The reference's ``bias_vl`` image-token
    # routing bias is dropped at load time (see module docstring).
