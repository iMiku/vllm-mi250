...
class DeepseekV4ModelArchConfigConvertor(ModelArchConfigConvertorBase):
    """Route a DeepSeek-V4 config carrying a vision tower to the VL wrapper.

    DeepSeek-V4-Flash-Vision-Exp ships the same architectures/model_type as the
    text-only DeepSeek-V4-Flash; the model-class resolver reads the raw
    ``hf_config.architectures``, so we mutate (not just override
    ``get_architectures``) only the stock text architecture when a vision tower
    is present. The VL wrapper marks the config copy it hands to the inner text
    ...
    def is_mm_prefix_lm(self, supports_multimodal: bool = True) -> bool:
        # The vision variant needs mm-prefix plumbing: it makes the scheduler
        # prefill image spans atomically (disable_chunked_mm_input) and routes
        # per-request image ranges to the sparse-SWA metadata builder, which
        # widens the sliding window bidirectionally in-kernel.
        if not supports_multimodal:
            return False
